...
F. 매듭 트랙 부활 (졸업 재설계): ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓  99%  🔄 (F-1~F-12·F-13 코드측·P1 전체 정리 완료 15커밋, F-13 공식 A-4 해제만 외부 임상 재검증 의존)
...
| ID | 작업 | 상태 | 의존 |
|---|---|---|---|
| F-1 | 마이그레이션 032·033·034 (cooling_period_days·knot_label·cycle_index·persona_codes·knot_archive·knot_revisit_schedule) | ✅ | `74abca9` |
| F-2 | `utils/knotPolicy.ts` + canEnterKnotTrack + 24 테스트 | ✅ | `4b190fa` |
| F-3 | `store/useKnotStore.ts` + AsyncStorage persist + `utils/knotCooldown.ts` + 9 테스트 | ✅ | `4c2c167` |
| F-4 | `(tabs)/_layout.tsx` 동적 탭 + `(tabs)/knot.tsx` placeholder + `link` 아이콘 | ✅ | `fec7093` |
| F-5 | `app/knot/prompt.tsx` 풀스크린 권유 모달 — 라벨 3종 분기 + canEnterKnotTrack 가드 + Android back 처리 | ✅ | `a0234a3` |
| F-6 | `utils/knotTrigger.ts` 6조건 AND + `hooks/useKnotTrigger.ts` + 홈 통합 + 18 테스트 + useEmotionalSafety hour 버그 동시 수정 | ✅ | `f56a568` |
| F-7 | graduation 6 화면 어휘 교체 + `useKnotPolicy` + `requestGraduation` 시그니처 확장 | ✅ | `8aa4d79` |
| F-8 | `utils/knotCycle.ts` + `api/knotArchive.ts` + cycle prompt 화면 + archive INSERT + 홈 trigger 가드 | ✅ | `41c298e`·`3ad7534` |
| F-11 | `lint:persona` 9개 어휘 룰 확장 + CoolingTimer 라벨 동적 + cooling/index 어휘 교체 | ✅ | `cee5e53` |
| F-9 | 회상 의식 스케줄러 — utils/knotRevisit + api/knotRevisit + revisit 화면 + 홈 trigger + 푸시 라우팅 (P05·P14 D+30/60, P06 D+7) | ✅ | `c587196` |
| F-10 | `knot_archive` view → [기록] 탭에 사이클 타임라인 노출 (`app/knot/archive.tsx` + RecordCard 조건부) | ✅ | `d388134` |
| F-12 | 통합 검증 — opus agent 6 시나리오 분석 → P0 4건 발견·즉시 fix (graduation 진입 가드·relationship hydrate·graduationLockGuard·cooling Day 동적화) | ✅ | `72a1326` |
| F-12-P1 | 통합 검증 P1 4건 정리 — useRevisitTrigger 안전·cancelRevisitsForCooling·cycle-prompt 위기 가드·useKnotCooldownReset | ✅ | `f1d9954` |
| F-12-P1-D | useKnotPolicy DB 우선 — useCoolingStore 4 필드 확장 + cooling 활성 시 DB 스냅샷 우선 + 홈 진입 시 graduation_cooling hydrate | ✅ | `9e85a25` |
| F-13 코드측 | CLAUDE.md 절대 규칙 표 갱신 — 매듭(구 졸업) 정책·매듭 권유 비허용·매듭 가역성·매듭 위기 게이트 4 항목 명문화 + 페르소나별 cooling 일수·라벨 명시 + 작업별 참조 문서 링크 | ✅ | (이번 커밋) |
| F-13 공식 해제 | A-4 *공식 해제* 표시 — 임상 재검증 통과 후 graduation-paused 화면 정리 + Phase F 100% 마감 | ⬜ | 외부 임상 재검증 |
