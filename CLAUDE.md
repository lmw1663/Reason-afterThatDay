...
| 위기 신호 | 3일 연속 mood_score 1~2점 또는 새벽(00:00~05:00, 즉 0~4시 inclusive) 진입 감지 시 반드시 `EmotionalCheckModal` 표시 — 핫라인 정보는 `resources/crisis-hotlines.json`에서만 로드, 하드코딩 금지. 페르소나별 우선 핫라인 매핑 적용. C-SSRS 양성 시 결정 트랙(분석·나침반) 자동 잠금 — 해제는 `/safety/release` 화면에서만 (24시간 경과 + 안전 4문항 통과) |
