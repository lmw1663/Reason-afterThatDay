...
| 유예 중 알림 | 유예기간 중 일반 알림(리마인더/감정변화) 전면 중지 |
| 판단 문구 | 진단/나침반 결과 화면에 "정답이 아니야" 문구 필수 포함 |
| 방향 변화 | 방향 변화에 대한 비난/판단 문구 금지 |
| D+N | D+N(이별 경과일) 전역 표시 항상 유지 |
| 위기 신호 | 3일 연속 mood_score 1~2점 또는 새벽(00~04시) 진입 감지 시 반드시 `EmotionalCheckModal` 표시 — 핫라인 정보는 `resources/crisis-hotlines.json`에서만 로드, 하드코딩 금지. 페르소나별 우선 핫라인 매핑 적용 |
