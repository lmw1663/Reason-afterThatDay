...
C-2 화면별 분기:        ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓░░░░░   80%  🔄 (G-11·G-5b·G-7c 잔존)
C-2-Ref 참고용 적용:    ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓  100%  ✅ 완료 (d614c74~1908c43)
C-3 다중 페르소나 충돌:   ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓░░░   85%  🔄 (4 화면 적용 완료, 단위 테스트만 잔존)
...
| C-2-G-5b | C5 about-me 신규 카테고리 (reality-check·body·needs·identity) | — | ⬜ | — |
...
| C-2-G-11 | 200 셀 단위 테스트 + dogfood 20명 | — | ⬜ | — |
...
| C-3-H-잔존 | journal/index.tsx 분기 마이그레이션 | — | ⬜ | — |
...
| ⏸️ G-8 | C8 쿨링 페르소나별 일수 (021 마이그레이션 + admin 탭) | 졸업 보류로 진입 제한 | A-4 해제 시 |
| ⏸️ Phase4-1 | 졸업 진입 화면 전체 (`app/graduation/*`) | 사용자 요청 | 임상 재검증 후 |
| ⏸️ X-2-B-3 | 졸업·스트림 ai-* 함수 페르소나 통합 | 졸업 보류 + 스트림 구조 별도 | A-4 해제 + 스트림 검토 |
...
**→ C-3-H 본 구현 + 4 화면 적용 완료.** 인프라 작업은 거의 마무리. 잔존: 신규 화면(G-7c·G-5b)·단위 테스트(G-11·C-3-H-test)·journal 분기 마이그레이션.
...
- C-3 + X-2-B: `fbcd106` X-2-B-1 → `6d60a23` C-3-H → `92a5f7b` X-2-B-2 → `5912578` C-3-H 4 화면 적용

## 다음 할 일 (우선순위 순)

1. **C-3-H journal 마이그레이션** — `app/journal/index.tsx` 분기들이 secondary persona도 검사하도록 (분기 다수, 중간 작업량)
2. **C-2-G-7c** 추억 트랙 잔여 — P08 부정 없는 봉인 / P15 짐 정리 의식 / P17 Continuing Bonds / P18 마주침 예측표 (신규 화면 4개, 큰 작업)
3. **C-2-G-5b** about-me 신규 카테고리 — reality-check / body / needs / identity (신규 화면 4개, 큰 작업)
4. **C-2-G-11 + C-3-H-test** 단위 테스트 인프라 셋업 (jest/vitest 미설치) + 200셀·8 케이스 검증
5. **D Phase** 검사 통합 (D-1~D-6) — B-0 라이선스 회신 후
6. **X-1·X-3·X-4** PIPA·임계 JSON·텔레메트리 (베타 진입 전 필수)
