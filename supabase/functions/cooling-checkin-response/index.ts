...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!lint.ok) {
      console.warn('[cooling-checkin-response] persona lint violation:', persona, lint.violations);
    ...
    return new Response(
      JSON.stringify({ response: appendAutoMessage(raw, persona) }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
    );
  } catch {
    const fallback = FALLBACK_BY_DAY[day] ?? FALLBACK_BY_DAY[1];
