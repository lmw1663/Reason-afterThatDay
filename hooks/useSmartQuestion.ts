...
export function useSmartQuestion(context: QuestionContext, currentDirection: Direction): Question | null {
  ...
  const candidates = pool
    ...
    .sort(
      (a, b) =>
        scoreQuestion(b, answered as Record<string, { updatedAt: string; status: string }>, currentDirection) -
        scoreQuestion(a, answered as Record<string, { updatedAt: string; status: string }>, currentDirection),
    );
